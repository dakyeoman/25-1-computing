...
from movement_processor_py import MovementDataProcessor  # 추가
...
def main():
    ...
    parser.add_argument('--use-movement-data', action='store_true', help='실제 이동 데이터 사용')
    parser.add_argument('--movement-dir', default='./movement_data', help='이동 데이터 디렉토리')
    ...
    try:
        ...
        # 이동 데이터 처리 (옵션)
        movement_data = None
        if args.use_movement_data:
            print("📊 실제 이동 데이터 로드 중...")
            try:
                movement_processor = MovementDataProcessor(data_dir=args.movement_dir)
                movement_data = movement_processor.build_flow_network_data(
                    areas, 
                    peak_hours=[7, 8, 9, 12, 18, 19]  # 출퇴근 + 점심시간
                )
                print(f"✅ 이동 데이터 로드 완료: {len(movement_data.get('area_stats', {}))}개 지역")
                
                # 이동 패턴 요약 출력
                print("\n📍 주요 이동 패턴:")
                movements = movement_data.get('movements', {})
                top_routes = []
                for from_area, dests in movements.items():
                    for to_area, count in dests.items():
                        if from_area in areas and to_area in areas:
                            top_routes.append((from_area, to_area, count))
                
                top_routes.sort(key=lambda x: x[2], reverse=True)
                for from_area, to_area, count in top_routes[:5]:
                    print(f"   {from_area} → {to_area}: {count:,}명/일")
                    
            except Exception as e:
                logger.warning(f"이동 데이터 로드 실패: {e}")
                print(f"⚠️ 이동 데이터를 사용할 수 없습니다. 기본 모드로 진행합니다.")
                movement_data = None
        
        # 데이터 수집 (이동 데이터 포함)
        collector = DataCollector()
        location_data, flow_network = collector.collect_data(areas, movement_data)
